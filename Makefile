...
ifeq ($(OS),Windows_NT)     # Windows
	...
	MOVE_FILES := for %%f in (.\build\*.elf .\build\*.uf2 .\build\*.hex) do (if exist %%f move %%f artifacts_to_upload)
	BUILD_SYS := ninja
	...
	ifeq ($(UNAME_S),Linux) # Linux
		...
		MOVE_FILES := mv build/*.elf build/*.uf2 build/*.hex artifacts_to_upload
